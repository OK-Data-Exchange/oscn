...
RSpec.describe Scrapers::NewCasesKpCounties do
  describe '#perform' do
    before do
    ...
    it 'creates a new case' do
      VCR.use_cassette 'daily_filings_alt_counties' do
        Sidekiq::Testing.inline! do
          Scrapers::NewCasesKpCounties.perform('Muskogee', '2010-01-04')
    ...
    it 'scrapes case but not html when equeue is false' do
      VCR.use_cassette 'daily_filings_alt_counties' do
        Sidekiq::Testing.inline! do
          Scrapers::NewCasesKpCounties.perform('Muskogee', '2010-01-04', enqueue: false)
          expect(CourtCase.first).not_to eq(nil)
